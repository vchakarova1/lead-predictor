document.addEventListener('DOMContentLoaded', () => {
  ...
  if (
    ...
    !leadResponseValue || !prospectResponseValue || !calculateButton ||
    !forecastHelper || !errorMessage ||
    Object.values(resultNodes).some((node) => !node) ||
    Object.values(progressBars).some((bar) => !bar) ||
    percentageNodes.length < 3 || !chartContainer || !chartBars.length
  ) {
    return;
  }

  function updateSliderLabels() {
    const leadRate = Number.parseFloat(inputs.leadResponseRate.value) || 0;
    const prospectRate = Number.parseFloat(inputs.prospectResponseRate.value) || 0;
